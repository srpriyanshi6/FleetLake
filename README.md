...
- **Real cleaning problems, solved**: raw ingestion is seeded with duplicate events, nulls, and malformed records (the kind at-least-once delivery pipelines actually produce); the staging job measurably fixes them — see logged drop counts below.
...
- **Runs end-to-end with one command**, no cloud account needed.
...
Both decisions directly control how much data an engine like Athena or BigQuery has to scan, and since those engines bill (and get slower) per byte scanned, this is a cost and performance decision, not just a style choice. This project is built to demonstrate that reasoning end to end, with numbers to back it up — not just describe it.
...
- Curated tables also carry `city` and `vehicle_id` as regular columns, not partition keys — Parquet's columnar storage + Athena's *predicate pushdown* still avoids reading unrelated column data even without partitioning on those fields (see the cost comparison below).

The partition column is dropped from the file contents and re-derived from the folder path — this is what lets a query engine prune whole files without opening them, purely by reading the S3 prefix / directory listing.
...
| Typical size vs. equivalent JSON | baseline | ~4–8x smaller in this project (see below) |

Since Athena and BigQuery bill **per byte scanned**, and scanning time is the biggest driver of query latency, a smaller, columnar, typed format is a direct cost and speed win — not just a "best practice" checkbox.
...
In the real AWS deployment, the exact same effect shows up as the "Data scanned" figure Athena prints under every query result — that's the number to screenshot when running `queries/analytical_queries.sql` against `fleetlake_raw` vs `fleetlake_curated` (see [Screenshots to add](#screenshots-to-add) below). On BigQuery, the query validator shows an estimated bytes-scanned figure live in the console as you type the query, before you even run it.
...
That single command populates `data_lake/raw`, `data_lake/staging`, and `data_lake/curated`, then runs and prints the 5 analytical queries and the before/after cost comparison. Takes under a minute.
...
The dashboard reads only local Parquet files (`data_lake/curated/`), so it deploys to [Streamlit Community Cloud](https://streamlit.io/cloud) with no database or secrets to configure:

1. Push this repo to GitHub, with `data_lake/curated/` committed (it's ~500 KB).
2. On share.streamlit.io, point a new app at this repo, main branch, `app/streamlit_app.py`.
3. Deploy — you get a public `https://<name>.streamlit.app` link to share.
